...
from engine import train_one_epoch, evaluate, eval_hter, get_EER
...
def get_args_parser():
    parser = argparse.ArgumentParser('HaTFAS Distillation', add_help=False)
...
def main(args):
    ...
    if args.eval:
        # CASIA-FASD and Replay-Attack
        EER, thr = get_EER(data_loader_val, model, device)
        print(f"* EER {EER*100:.2f}% thr {thr:.2f}")
        # Replay-Attack and OULU-NPU
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        #     min_EER_thr = thr
            ...
            if args.output_dir:
                checkpoint_paths = [output_dir / 'min_hter_checkpoint.pth']
        ...
        # CASIA-FASD
        # print(f"Min EER: {min_EER*100:.2f}%")
        ...
        result_list = [max_accuracy, FAR*100, FRR*100, HTER*100, min_HTER*100]
        
        # CASIA-FASD
        # result_list = [max_accuracy, EER*100, thr, min_EER*100, min_EER_thr]

        if args.output_dir and utils.is_main_process():
            with (output_dir / "log.txt").open("a") as f:
                f.write(json.dumps(log_stats) + "\n")
                f.write('result_list:'+str(result_list)+"\n")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('HaTFAS training and evaluation script', parents=[get_args_parser()])
